...
describe("ollamaProvider", () => {
  ...
  it("composes with a JSON schema format and validates the result", async () => {
    ...
    const r = await ollamaProvider({ model: "m", baseUrl: "http://x" }, fetch).compose(image, []);
  ...
  it("retries once on malformed output, then reports schema mismatch", async () => {
    ...
    await expect(ollamaProvider({ model: "m", baseUrl: "http://x" }, fetch).compose(image, [])).rejects.toMatchObject({ status: 422, code: "schema_mismatch" });
    expect(calls).toBe(2);
  });

  it("maps a missing model to ollama_model_missing", async () => {
    const fetch = fakeFetch(() => ({ status: 404, text: '{"error":"model \\"nope\\" not found"}' }));
    await expect(ollamaProvider({ model: "nope", baseUrl: "http://x" }, fetch).answerQuestion(image, [], "q?")).rejects.toMatchObject({ code: "ollama_model_missing" });
  });
  ...
  it("maps a text-only model rejecting the image to 422 ollama_no_vision", async () => {
    ...
    await expect(ollamaProvider({ model: "qwen3", baseUrl: "http://x" }, fetch).compose(image, [])).rejects.toMatchObject({ status: 422, code: "ollama_no_vision" });
  });
  ...
  it("maps connection failures to ollama_unreachable with the Docker hint", async () => {
    ...
    await expect(ollamaProvider({ model: "m", baseUrl: "http://localhost:11434" }, fetch).answerQuestion(image, [], "q?")).rejects.toMatchObject({
